...
import { updateWeeklySchedule } from '@/lib/storage';
...
export const ScheduleSection = ({ user, onUpdate }: ScheduleSectionProps) => {
  ...
  const applyProgram = (days: number[]) => {
    setSelectedDays(days);
    const updatedUser = updateWeeklySchedule(user, days);
    onUpdate(updatedUser);
  };

  const saveSchedule = () => {
    const updatedUser = updateWeeklySchedule(user, selectedDays);
    onUpdate(updatedUser);
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Pilih hari-hari untuk berlatih
        </CardDescription>
      </CardHeader>
          ...
          <div className="flex flex-wrap gap-2">
            {WEEKLY_PROGRAMS.map((program) => (
              <Button
                ...
                className={cn(
                  "text-xs",
                  JSON.stringify(selectedDays) === JSON.stringify(program.days) && 
