...
interface WorkoutTimerProps {
  ...
  onComplete: (exercises: string[], duration: number, category: string) => void;
  onClose: () => void;
}
...
export const WorkoutTimer = ({ category, onComplete, onClose }: WorkoutTimerProps) => {
  ...
  const handleComplete = () => {
    ...
    onComplete(exercises.map(e => e.id), duration, category);
  };
    ...
    return (
      <div className="fixed inset-0 bg-background z-50 flex items-center justify-center p-4">
        <Card className="w-full max-w-md p-8 text-center animate-scale-in">
          ...
          <h2 className="text-3xl font-fredoka font-bold text-foreground mb-2">Hebat Sekali!</h2>
          <p className="text-lg text-muted-foreground mb-6">
            Kamu sudah menyelesaikan latihan {getCategoryLabel(category)}!
          </p>
          <div className="flex gap-3">
            <Button variant="outline" onClick={onClose} className="flex-1" size="lg">Kembali</Button>
    ...
    <div className="fixed inset-0 bg-background z-50 flex flex-col">
      <div className="flex items-center justify-between p-4 border-b">
        ...
        <div className="text-center">
          <div className="text-sm text-muted-foreground">Latihan {currentExerciseIndex + 1} dari {totalExercises}</div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6">
        <div className={cn("px-4 py-2 rounded-full text-sm font-bold text-white mb-4", phaseColors[phase])}>
          {phaseLabels[phase]}
        </div>
        <div className={cn("w-48 h-48 rounded-full flex items-center justify-center mb-6", phase === 'ready' && 'animate-countdown-pulse', phaseColors[phase])}>
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground max-w-md">{currentExercise.description}</p>
        </div>
        {phase === 'exercise' && (
      ...
      <div className="p-6 border-t bg-card">
        <div className="flex justify-center gap-4">
          <Button variant="outline" size="lg" onClick={skipToNext} className="w-32">
            <SkipForward className="w-5 h-5" /> Lewati
          </Button>
          <Button size="xl" onClick={() => setIsPaused(!isPaused)} className="w-40">
