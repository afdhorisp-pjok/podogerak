...
import { Exercise, getDomainLabel } from '@/lib/workoutData';
import { Play, Pause, SkipForward, X, Volume2, VolumeX, CheckCircle, Info } from 'lucide-react';
import { cn } from '@/lib/utils';

interface SessionRunnerProps {
  exercises: Exercise[];
  onComplete: (exercises: string[], duration: number, domain: string) => void;
  onClose: () => void;
}
      ...
      <div className="fixed inset-0 bg-background z-50 flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6">
          ...
          <span className="px-3 py-1 rounded-full text-xs font-bold bg-primary/10 text-primary mb-4">
            {getDomainLabel(currentExercise.domain)} • {currentExercise.duration} detik
          </span>
          ...
          <Card className="w-full max-w-md p-4 mb-4 border-2 border-primary/20 bg-primary/5">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{currentExercise.parentInstruction}</p>
              </div>
            </div>
          </Card>
    ...
    <div className="fixed inset-0 bg-background z-50 flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6">
        ...
        <div className="text-center mb-8">
          ...
          {phase === 'exercise' && (
            <p className="text-lg text-primary font-bold">{currentExercise.childInstruction}</p>
          )}
        </div>
      </div>
